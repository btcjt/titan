...
//! 1. Relay discovery (kind 10002) → learn the pubkey's preferred relays
//! 2. Manifest fetch (kind 15128/35128) → get the site's file→hash mapping
//! 3. Path resolution → look up the requested path in the manifest
//! 4. Blob fetch → download from Blossom servers with SHA256 verification
//!
...
impl Resolver {
    ...
    pub async fn resolve(
        ...
        let manifest = self.get_manifest(&public_key, &pubkey_hex).await?;
    ...
    async fn get_manifest(
        ...
        // Check cache
        if let Some(cached) = self.cache.get_manifest(pubkey_hex) {
            if let Ok(event) = serde_json::from_slice::<Event>(&cached) {
                if let Ok(manifest) = Manifest::from_event(&event) {
                    debug!("using cached manifest for {pubkey_hex}");
                    return Ok(manifest);
                }
            }
        }
